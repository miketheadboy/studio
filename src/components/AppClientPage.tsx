...
const AppClientPage: FC = () => {
  ...
  const [musePrompt, setMusePrompt] = useState("Awaiting inspiration...");
  ...
  const [artistInspiration, setArtistInspiration] = useState("Artist examples will show up here...");
  ...
  const [rephrasedLine, setRephrasedLine] = useState("");
  const [lyricalLabLoader, setLyricalLabLoader] = useState(false);

  const [lyricsToAnalyze, setLyricsToAnalyze] = useState("");
  const [lyricalMood, setLyricalMood] = useState("Lyric mood analysis...");
  ...
  const [metaphorResult, setMetaphorResult] = useState("");
  const [metaphorLoader, setMetaphorLoader] = useState(false);

  const [rhymeSchemeInputText, setRhymeSchemeInputText] = useState("");
  const [rhymeSchemeResult, setRhymeSchemeResult] = useState("Rhyme scheme ideas will appear here...");
  ...
  const [chordProgression, setChordProgression] = useState("Select a mood to see progressions...");
  ...
  const [reharmonizedChords, setReharmonizedChords] = useState("");
  ...
  // Placeholder for stdProgPattern related states if implemented
  const [stdProgResult, setStdProgResult] = useState("");
  const [stdProgLoader, setStdProgLoader] = useState(false);

  const [randomProgFeel, setRandomProgFeel] = useState("");
  const [randomProgResult, setRandomProgResult] = useState("");
  const [randomProgLoader, setRandomProgLoader] = useState(false);

  const [chordsForVoicing, setChordsForVoicing] = useState("");
  const [voicingSuggestions, setVoicingSuggestions] = useState("Chord voicing ideas...");
  ...
  const [rawPhrases, setRawPhrases] = useState<Phrase[]>([]); // Will be populated by useEffect
  ...
  const SONGWRITING_TIPS = [
      "Try the Phrase Spark for a new line!", "Need a title? Visit Album Title Alley!", "How about a Dylan quote for inspiration from the Poet's Corner?", "Experiment with a new mood in the Chord Corner.", "Generate a random chord progression in The Chord Forge.", "Reharmonize an existing progression with The Chord Alchemist.", "Feeling blocked? Summon The Muse's Whisper for a thematic idea.", "Just start writing in The Songwriter's Canvas, even if it's nonsense. You can refine it later.", "Use the Song Structure tools to outline your song in the Canvas.", "Try searching for a specific keyword in the Phrase Spark.", "Listen to a song in a style you're aiming for, then come back fresh.", "Need ideas for a bridge? Try the Bridge Builder Prompts.", "What's the core emotion of your song? Explore it with the Lyrical Mood Ring.", "Thinking about the melody? Check out the Melody Maker's Memo.", "Need a beat? The Rhythm Architect has ideas!", "Stuck on a phrase? Use the Word Lab to rephrase it in a different style!", "Build powerful imagery with The Alchemist's Wordsmith (Metaphor Builder)!", "Get some Sound Palette (Instrumentation) ideas for your song's feel!", "Find inspiration from your favorite mentors in Mentor's Corner!", "Study a song to learn new techniques from Mentor's Discography!", "For a complete starting point, try the AI Song Draft Generator!"
  ];
  const CHORD_PROGRESSIONS: Record<string, { name: string; progressions: string[] }> = { 
      bluesySwagger: { name: "Bluesy Swagger", progressions: ["I - IV - V (e.g. A - D - E)", "I - I - I - I <br>IV - IV - I - I <br>V - IV - I - V (12-bar)", "I - vi - ii - V (e.g. C - Am - Dm - G)"] },
      punkFury: { name: "Alt-Rock Edge", progressions: ["I - IV - V (Loud & Fast!)", "I - V - vi - IV (e.g. G - D - Em - C)"] },
      cosmicBallad: { name: "Folk/Americana Ballad", progressions: ["Am - G - C - F", "C - G/B - Am - F", "Em - C - G - D"] },
      folkStoryteller: { name: "Folk Storyteller", progressions: ["G - C - D - G", "Am - G - C - G", "C - F - G - C"] },
      dreamyPop: { name: "Indie Folk Dream", progressions: ["Imaj7 - IVmaj7 - ii7 - V7", "vi7 - IVmaj7 - Imaj7 - V7sus"] },
      grittyFunk: { name: "Roots Rock Groove", progressions: ["Em7 - A7 (loop)", "i7 - IV7 (loop, e.g. Am7 - D7)"] },
      wistfulAcoustic: { name: "Wistful Acoustic", progressions: ["C - G/B - Am - Em/G - F - C/E - Dm7 - Gsus - G", "G - D/F# - Em - C"] },
      dustBowlBallad: { name: "Dust Bowl Ballad", progressions: ["Am - Dm - Am - E7", "i - iv - V7 - i (e.g. Em - Am - B7 - Em)"] },
      appalachianFolk: { name: "Appalachian Folk", progressions: ["I - V - I (e.g. G - D - G)", "I - IV - I - V - I (e.g. D - G - D - A - D)"] },
      roadhouseRocker: { name: "Roadhouse Rocker", progressions: ["E - A - B7", "I - bVII - IV - I (Classic Rock feel, e.g. A - G - D - A)"] }
  };
  ...
  const handleGetAIPhrases = async () => {
    ...
    try {
        ...
        if (result.success && result.data) {
            ...
            setRawPhrases(prev => [...prev, ...newPhrasesToAdd]);
  ...
  const handleToolAction = async (
    ...
    if (result.success && result.data) {
      ...
      else if (result.data.songSuggestion && result.data.studyPlan && result.data.keyTakeaways) { // for study song
         content = `Song Suggestion: ${result.data.songSuggestion}\n\nHow to Study It:\n${result.data.studyPlan}\n\nKey Takeaways:\n${result.data.keyTakeaways}`;
      }
      else if (result.data.rephrasedLine) content = result.data.rephrasedLine;
      else if (result.data.moodAnalysis) content = result.data.moodAnalysis;
      else if (result.data.metaphor) content = result.data.metaphor;
      else if (result.data.rhymeScheme) content = result.data.rhymeScheme;
      else if (result.data.expandedSection) content = result.data.expandedSection;
      else if (result.data.chordProgression) content = result.data.chordProgression;
      else if (result.data.reharmonizedChords) content = result.data.reharmonizedChords;
      else if (result.data.suggestions) content = result.data.suggestions; // for voicing
      else if (result.data.instruments) content = result.data.instruments; // for instrumentation
      else if (result.data.structure) content = result.data.structure; // for song structure
      else content = JSON.stringify(result.data, null, 2); // fallback for complex objects
      setter(content);
    } else {
          ...
          <AccordionContent className="pt-4">
            ...
            <div className="mt-4 text-sm text-muted-foreground">
              ...
              <ul className="list-disc list-inside ml-4 mt-1 space-y-1">
                ...
                <li><strong>Rewrite & Refine:</strong> Your first draft is just the beginning. The "Word Lab" and "Chord Alchemist" can offer new perspectives.</li>
                <li><strong>Listen Deeply:</strong> To the world around you, to other artists ("Mentor's Corner"), and to the song taking shape within you.</li>
              </ul>
            </div>
          </AccordionContent>
        </AccordionItem>
          ...
          <AccordionContent className="space-y-8 pt-4">
            ...
            <Card className="tool-card">
              ...
              <CardContent>
                ...
                <div className="space-y-2 mb-3">
                  <Label htmlFor="wordAssociationInput">Your turn:</Label>
                ...
                <CardContent>
                  ...
                  <div className="space-y-2 mb-3">
                    ...
                    <Select value={rephraseStyle} onValueChange={setRephraseStyle}>
                      <SelectTrigger><SelectValue placeholder="Default Rephrase" /></SelectTrigger>
